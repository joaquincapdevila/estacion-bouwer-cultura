...
import { stuff } from "../lib/placeholder-data";
  ...
  return (
    <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {stuff.map((data) => (
        <li
          key={data.name}
          className="w-full max-w-sm p-6 col-span-1 gap-3 border hover:border-red-950 hover:scale-95 hover:cursor-pointer transition-all border-red-500 bg-gradient-to-tr to-red-300 from-red-100 rounded-2xl text-red-950">
          <div className="flex flex-col items-center gap-4 pb-4">
            <Image
              ...
              height={40}
            />
            <h3 className="mb-1 text-xl font-medium">{data.name}</h3>
            <div className="flex items-center gap-2">
              {/* <AwardIcon className="h-5 w-5 text-muted-foreground" /> */}
              <p className="text-sm">{data.cargo}</p>
            </div>
          </div>
          <div className="text-sm text-center">{data.email}</div>
        </li>
      ))}
    </ul>
  );
...
export function CardStatics(props) {
  return (
